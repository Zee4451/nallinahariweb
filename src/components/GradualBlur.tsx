...
const GradualBlur: React.FC<PropsWithChildren<GradualBlurProps>> = props => {
  ...
  useEffect(() => {
    if (!config.autoHideAtBottom || config.position !== 'bottom' || config.target !== 'page') {
      return;
    }
    ...
    const checkScrollPosition = () => {
      ...
      const threshold = config.bottomThreshold || 60;
  ...
  const containerStyle: CSSProperties = useMemo(() => {
    ...
    const isEffectiveVisible = isVisible && !isNearBottom;
    ...
    return baseStyle;
  }, [config, responsiveHeight, responsiveWidth, isVisible, isNearBottom]);
